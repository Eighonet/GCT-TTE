...
parser.add_argument("-gn", "--group-name", type=str, default="MTTE_ratio",
                    help="group name for wandb")
...
def train_run():
    ...
    epoch_loss_sr = []
    epoch_loss_rmse = []
    epoch_test_loss_mape = []
    epoch_test_loss_mae = []
    epoch_test_loss_sr = []
    epoch_test_loss_rmse = []
    ...
    for epoch in range(EPOCHS):
        ...
        model.train()
        for index, (graph_embed, labels, imgs_embed, extras, lengths) in enumerate(tqdm(train_dataloader)):
            l = train(model, optimizer, DEVICE, data, graph_embed, labels, imgs_embed, extras, lengths)
            train_loss_mape.append(l[0])
            train_loss_mae.append(l[1])
            train_loss_sr.append(l[2])
            train_loss_rmse.append(l[3])
        epoch_loss_mape.append(stat.mean(train_loss_mape))
        epoch_loss_mae.append(stat.mean(train_loss_mae))
        epoch_loss_sr.append(stat.mean(train_loss_sr))
        epoch_loss_rmse.append(stat.mean(train_loss_rmse))
        ...
        epoch_test_loss_mape.append(stat.mean(val_loss_mape))
        epoch_test_loss_mae.append(stat.mean(val_loss_mae))
        epoch_test_loss_sr.append(stat.mean(val_loss_sr))
        epoch_test_loss_rmse.append(stat.mean(val_loss_rmse))
        
        temp_dict["train/MAPE"] = epoch_loss_mape
        temp_dict["train/MAE"] = epoch_loss_mae 
        temp_dict["train/SR"] = epoch_loss_sr
        temp_dict["train/RMSE"] = epoch_loss_rmse
